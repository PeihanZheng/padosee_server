...
module.exports = {
    ...
    getAlertByUserId: (id, callback) => {
        pool.query(`SELECT alerts_list.alerts_id, alerts_list.time_detected, alerts_list.analytics_id, alerts_list.cam_id, camera_list.cam_name FROM alerts_list INNER JOIN camera_list ON alerts_list.cam_id = camera_list.cam_id WHERE camera_list.user_id = ?`, [id], (error, results, fields) => {
            if (error) {
                return callback(error);
            } else {
                return callback(null, results);
